...
[ "$N" -gt 0 ] || { echo "no .safetensors in $MODEL, run fetch-model.sh first"; exit 1; }
...
ls -la "$TRUNK"
echo
echo "next: ./bin/litmoe $MODEL --trunk $TRUNK --preset server --tok $MODEL --prompt 'Hello' --incremental"
